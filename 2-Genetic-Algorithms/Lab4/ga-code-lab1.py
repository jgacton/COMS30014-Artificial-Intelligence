...
def mutate(pop, mutation, alphabet, elitism):
    ...
    length = len(pop[start_index]["solution"])
    for i in pop[start_index:]:
...
def cross(mum, dad, uniform):
    if uniform:
        sol = ""
        for i in range(0,len(mum)):
...
def do_the_ga(pop_size=100, tournament_size=2, crossover=0.0, uniform=False,
    ...
    while generation < max_gen and best["fitness"] < 1:
        convergence=matches(pop[0]["solution"],pop[len(pop)-1]["solution"])/len(pop[0]["solution"])
        print("Generation:{:.1d}, convergence:{:.4f}", generation,convergence)
        generation += 1
...
def q2a():
    xs = []
    ys = []
    for tournament_size in [2,3,4,5,6,7,8,9,10]:
        best=do_the_ga(tournament_size=tournament_size)
        xs.append(tournament_size)
        ys.append(best[1]["fitness"])
...
def q2b():
    ...
    xs = []
    ys = []
    for target in ["methinks it is like a very furry fuzzy wuzzy weasel", "to be or not to be", "hello world", "methinks it is not a weasel"]:
        best=do_the_ga(tournament_size=tournament_size, target=target)
        xs.append(len(target))
        ys.append(best[1]["fitness"])
...
def q2c():
    xs=[]
    ys=[]
    for alphabet in ["abcdefghijklmnopqrstuvwxyz ","abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ123456789!@£#$%^&*()[]\{\}-_"]:
        best=do_the_ga(alphabet=alphabet)
        xs.append(len(alphabet))
        ys.append(best[1]["fitness"])
...
def q2d():
    ...
    for size in [8,16,32,64,128,256,512,1024,2048,4096]:
        best=do_the_ga(pop_size=size, tournament_size=2)
        xs.append(size)
        ys.append(best[1]["fitness"])
...
def q2e():
    xs=[]
    ys=[]
    for r in [0.05/28,0.1/28,0.2/28,0.33/28,0.5/28,1/28,2/28,3/28,4/28,7/28,10/28]:
        best=do_the_ga(mutation=r)
        xs.append(r)
        ys.append(best[1]["fitness"])
...
def q3a():
    gens, best=do_the_ga()
    print("No elitism: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))
    gens, best=do_the_ga(elitism=True)
    print("Elitism: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))


def q3b():
    gens, best=do_the_ga()
    print("No converged initial pop: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))
    gens, best=do_the_ga(converged=True)
    print("Converged initial pop: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))

def q3c():
    gens, best=do_the_ga()
    print("No crossover: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))
    gens, best=do_the_ga(crossover=0.2)
    print("Crossover=0.2: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))
    gens, best=do_the_ga(crossover=0.2,uniform=True)
    print("Uniform crossover: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))

def q4():
    gens, best=do_the_ga()
    print("Looking at convergence: {:4d} generations yielded: '{}' ({:.3f})".format(gens, best["solution"], best["fitness"]))
