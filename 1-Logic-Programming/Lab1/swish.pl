...
released_after(M1, M2) :-
    movie(M1, Y1),
    movie(M2, Y2),
    Y1 > Y2.

released_before(M1, M2) :-
    movie(M1, Y1),
    movie(M2, Y2),
    Y1 < Y2.
...
co_star(A1, A2) :-
    (actor(M1, A1, _) ; actress(M1, A1, _)),
    (actor(M2, A2, _) ; actress(M2, A2, _)),
    M1 == M2.

plays(M, A, R) :-
    (actor(M, A, R) ; actress(M, A, R)).
...
self_direct(AD) :-
    director(M1, AD),
    (actor(M2, AD, _) ; actress(M2, AD, _)),
    M1 == M2.

cameo(M, A) :-
    (actor(M, A, C) ; actress(M, A, C)),
    A == C.
...
multi_role(M, A) :-
    (actor(M, A, C1) ; actress(M, A, C2)),
    (actor(M, A, C2) ; actress(M, A, C2)),
    not(C1 = C2).

veteran(A) :-
    (actor(M1, A, _) ; actress(M1, A, _)),
    (actor(M2, A, _) ; actress(M2, A, _)),
    movie(M1, Y1),
    movie(M2, Y2),
    Y2 > (Y1 + 25).
...
actor(covid19, nirav_ajmeri, the_hero).
actor(covid19, nirav_ajmeri, the_villain).
movie('2nd_wave', 2020).
...
movie(test, 2020).
actor(test, laurence_fishburne, c).
...
actor(no_country_for_old_men, javier_bardem).
